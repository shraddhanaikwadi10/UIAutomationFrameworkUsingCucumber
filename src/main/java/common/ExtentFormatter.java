...
/**
 * Cucumber formatter for formatting Extent Reports. See
 * <a href="http://extentreports.com/">Extent reports</a> for more information.
 */
...
public class ExtentFormatter implements Formatter {
	...
	private void handleTestStepStarted(TestStepStarted event) {
		...
		if (TestSourcesModel.isBackgroundStep(testSources.getAstNode(currentFeatureFile, testStep.getStepLine()))) {
			...
			try {
				ExtentTest node = backgroundThreadLocal.get().createNode(new GherkinKeyword(keywordFromSource),
						testStep.getStepText());
				stepTestThreadLocal.set(node);
			} catch (ClassNotFoundException e) {
				System.out
						.println("Something went wrong while trying to parse the Gherkin keyword for this step: " + e);
			}
		}
		if (!scenarioOutlineTest) {
			...
			if (scenarioTest != null) {
				...
				try {
					if (!TestSourcesModel
							.isBackgroundStep(testSources.getAstNode(currentFeatureFile, testStep.getStepLine()))) {
						testNode = scenarioTest.createNode(new GherkinKeyword(keywordFromSource),
								testStep.getStepText());
						stepTestThreadLocal.set(testNode);
					}
				} catch (ClassNotFoundException e) {
					System.out.println(
							"Something went wrong while trying to parse the Gherkin keyword for this step: " + e);
				}
			}
		}
	}
	...
	private void handleStartOfFeature(TestCase testCase) {
		if (currentFeatureFile == null || !currentFeatureFile.equals(testCase.getUri())) {
			...
			ExtentTest feature = getExtentReport().createTest(cucumberFeature.getName(),
					cucumberFeature.getDescription());
	...
	private void handleBackground(TestCase testCase) {
		Background background = TestSourcesModel
				.getBackgoundForTestCase(testSources.getAstNode(currentFeatureFile, testCase.getLine()));
		if (background != null) {
			if (scenarioOutlineTest) {
	...
	private void handleScenarioOutline(TestCase testCase) {
		if (TestSourcesModel
				.isScenarioOutlineScenario(testSources.getAstNode(currentFeatureFile, testCase.getLine()))) {
			ScenarioOutline scenarioOutline = (ScenarioOutline) TestSourcesModel
					.getScenarioDefinition(testSources.getAstNode(currentFeatureFile, testCase.getLine()));
	...
	private void handleScenario(TestCase testCase) {
		...
		if (scenarioOutlineTest) {
			scenarioOutlineTest = false;
			ScenarioDefinition scenarioDefinition = TestSourcesModel
					.getScenarioDefinition(testSources.getAstNode(currentFeatureFile, testCase.getLine()));
			scenario = scenarioOutlineThreadLocal.get().createNode(Scenario.class, scenarioDefinition.getName());
		} else {
			gherkin.ast.Scenario scenarioDefinition = (gherkin.ast.Scenario) TestSourcesModel
					.getScenarioDefinition(testSources.getAstNode(currentFeatureFile, testCase.getLine()));
			scenario = featureTestThreadLocal.get().createNode(Scenario.class, scenarioDefinition.getName(),
	...
	private static File getReportsDir(File file) {
		File reportsDir = new File(DEFAULT_REPORT_DIRECTORY + File.separator + System.currentTimeMillis()
